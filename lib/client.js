...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * 等输入框出现。
     *
     * 会话切换后输入框是异步挂载的：fork/open 返回时 DOM 里往往还没有它，
     * 直接调用 findComposer() 会拿到 null，于是报「新会话里没找到输入框」。
     * 所以写入前先轮询等待。
     *
     * @param timeoutMs - 最长等待时间。
     * @returns 输入框元素，或 null。
     */
    ...
    async function setComposerText(text) {
      ...
      // 会话刚切过来时输入框可能还没挂上，先等它。
      if (findComposer() === null) {
        diag('setComposerText waiting for composer')
    ...
    function turnInfoOf(node) {
      ...
      const turn = location && location.turn
      // 真机上这些结构是精简引用，字段名不固定：
      //   轮次编号可能在 turn.turn / turn 自身 / location.turnIndex
      //   完成边界可能在 turn.end.seq / turn.endSeq / location.turnEndSeq
      // 逐个试，任一命中即可——这决定 fork 锚点能不能算出来。
      const number = [turn && turn.turn, turn, location && location.turnIndex]
        .map((value) => Number(value))
        .find((value) => Number.isFinite(value))
      if (number === undefined) return null
      const endSeq = [
        turn && turn.end && turn.end.seq,
        turn && turn.endSeq,
        location && location.turnEndSeq,
        node.turnEndSeq,
      ].map((value) => Number(value)).find((value) => Number.isFinite(value))
      return {
        turn: number,
        status: (turn && turn.status) ?? (location && location.status),
        endSeq: endSeq === undefined ? null : endSeq,
      }
    }
    ...
    function forkBoundaryBefore(node, prevUserSeq) {
      ...
      if (targetTurn) {
        ...
        for (const host of userMessageHosts()) {
          ...
          // 只要求「有 turn/end」——不再额外要求 status === 'closed'。
          // 那个状态字符串是内部枚举，写法一变（completed/done/…）就会把所有
          // 候选跳过，边界恒为 null，于是每次编辑都降级成「回填 + 手动发送」。
          // endSeq 存在本身就说明这一轮已经结束了，拿它当锚点就是对的。
          if (!Number.isFinite(candidate.endSeq)) continue
    ...
    function pendingItemFor(source, node, text) {
      ...
      // 2) 文本匹配：只认**规范化后全等**。
      //
      //    这里一度用过「包含」关系（itemText.includes(needle) 或反过来），
      //    后果很严重：编辑常常就是把长文本删短（「A，B」→「A」），短文本必然
      //    包含在队列里某条长文本中，于是**已消费的历史消息被误认成队列消息**，
      //    走了原地改写、不分叉——现象就是「旧消息还在、修改后的内容在排队」。
      //
      //    误判成队列项的代价远大于漏判：漏判只会让一条真·排队消息走分叉，
      //    而误判会让历史消息编辑完全失效。所以只认全等。
      ...
      if (needle !== '') {
        const same = queued.filter((item) => {
          const itemText = (item.text === null || item.text === undefined ? item.preview || '' : item.text)
            .replace(/\s+/g, ' ')
            .trim()
          return itemText !== '' && itemText === needle
        })
        if (same.length === 1) return same[0]
      }

      // 3) 兜底：界面上这条就是最后一条用户消息、且队列里只有一项待发时按位置认领。
      //    这是「刚发出去还没落地就改」的常见场景，但仍然要求文本全等，
      //    否则同样会把历史消息误认成队列项。
      if (queued.length === 1 && isLastUserMessage(node) && needle !== '') {
        const only = queued[0]
        const onlyText = (only.text === null || only.text === undefined ? only.preview || '' : only.text)
          .replace(/\s+/g, ' ')
          .trim()
        if (onlyText === needle) return only
      }
      return null
    }
    ...
    function startObserving() {
      ...
      observer = new MutationObserver(() => {
        // 只在有新增节点时补，避免无谓抖动。
        ensureEditButtons()
        // 改写后的视觉镜像要盖住 React 后续渲染出来的旧行（rAF 节流）。
        scheduleMirror()
      })
    ...
    //#region DOM 镜像（同一会话内的视觉改写）
    /**
     * DSH 的 transcript 按设计是只追加的：
     * `dsh-client-ui-chat` 的注释写明 replacement 事件只进模型投影
     * （"replacement copies stay model-only"），用户气泡定义只匹配
     * append 事件。所以“旧的不再显示”只能由插件在 DOM 层完成：
     * 藏掉被盖住的 flow，把被编辑那行的气泡换成新文本。
     * 记录落 localStorage，刷新后重放，保证所见与模型一致。
     */
    const MIRROR_KEY = 'dsh-edit-resend.mirror.v2'
    // React 重渲染会整片替换 DOM（内联隐藏跟着丢），所以 observer 每次触发
    // 都全量重放一次，用 rAF 节流；绝不用“flow 数量没变就跳过”这类 guard，
    // 节点被替换后数量不变但标记全丢，正是之前“编辑完旧的又回来”的原因。
    ...
    function saveMirror(store) {
      try {
        localStorage.setItem(MIRROR_KEY, JSON.stringify(store))
      } catch (error) { /* 存不下就只影响刷新后的重放 */ }
    }

    /**
     * 取某会话的镜像状态。老版本存的是纯数组，做一次迁移。
     * @returns {{edits: Array, remap: Record<string, number>}}
     */
    function sessionMirror(store, sessionId) {
      const raw = store[sessionId]
      if (Array.isArray(raw)) return { edits: raw, remap: {} }
      if (raw && typeof raw === 'object') {
        return {
          edits: Array.isArray(raw.edits) ? raw.edits : [],
          remap: raw.remap && typeof raw.remap === 'object' ? raw.remap : {},
        }
      }
      return { edits: [], remap: {} }
    }
    ...
    /**
     * 同一行反复改写时，fiber 上的 seq 永远是最初那条的 seq，而它早已被盖掉。
     * 用 remap 把最初 seq 指到最新的替换事件 seq，保证第二次改写还能定位。
     */
    function mirrorEffSeq(sessionId, seq) {
      if (typeof sessionId !== 'string' || sessionId === '' || !Number.isFinite(seq)) return seq
      const { remap } = sessionMirror(loadMirror(), sessionId)
      let current = seq
      for (let i = 0; i < 10; i++) {
        const next = remap[String(current)]
        if (!Number.isSafeInteger(next) || next === current) break
        current = next
      }
      return current
    }

    function recordMirrorEdit(sessionId, { atSeq, domSeq, turn, hideUntil, text, replacementSeq }) {
      if (typeof sessionId !== 'string' || sessionId === '') return
      const store = loadMirror()
      const mirror = sessionMirror(store, sessionId)
      // 索引快照：提交时刻 editedKind 在顶层 kinds 里的下标 + kinds 总数。
      // React 重渲染会替换节点，但列表是只追加的，老节点下标不变；
      // 重放时只藏 [editIdx, kindCount)，之后新长的 turn 下标更大，自然豁免。
      // 这条完全不依赖 turn 标记，是 turn 读不到时的兜底。
      let editIdx = null
      let kindCount = null
      try {
        const kinds = topKinds()
        kindCount = kinds.length
        const host = Number.isFinite(domSeq) || Number.isFinite(atSeq)
          ? hostBySeq(Number.isFinite(domSeq) ? domSeq : atSeq)
          : null
        const kind = kindOf(host)
        const idx = kinds.indexOf(kind)
        editIdx = idx === -1 ? null : idx
      } catch (error) {
        editIdx = null
        kindCount = null
      }
      mirror.edits.push({
        atSeq,
        domSeq: Number.isFinite(domSeq) ? domSeq : atSeq,
        turn: Number.isFinite(turn) ? turn : null,
        // 藏到编辑时刻的最大轮次为止：之后新产生的轮次（重答、用户新消息）
        // turn 更大，必须豁免，否则“新回答刚出来就被藏掉”。
        hideUntil: Number.isFinite(hideUntil) ? hideUntil : null,
        editIdx,
        kindCount,
        text,
        time: Date.now(),
      })
      mirror.edits = mirror.edits.slice(-20)
      if (Number.isSafeInteger(replacementSeq)) {
        // domSeq（fiber 定位用）和 atSeq（本次实际发送的）都指到最新替换。
        mirror.remap[String(domSeq)] = replacementSeq
        mirror.remap[String(atSeq)] = replacementSeq
      }
      store[sessionId] = mirror
      saveMirror(store)
    }

    /** 当前可见会话的 id（取第一条用户消息的归属）。 */
    function visibleSessionId() {
      const hosts = userMessageHosts()
      if (hosts.length === 0) return null
      try {
        return inspectConversation(hosts[0]).sessionId || null
      } catch (error) {
        return null
      }
    }

    function clearMirrorMarks() {
      for (const el of document.querySelectorAll('[data-der-hidden]')) {
        el.style.display = ''
        el.removeAttribute('data-der-hidden')
      }
      for (const el of document.querySelectorAll('[data-der-retext]')) {
        const bubble = el.querySelector('[class*="userBubble"],[class*="_bubble"]')
        if (bubble !== null && el.dataset.derOrig !== undefined) bubble.textContent = el.dataset.derOrig
        el.removeAttribute('data-der-retext')
        delete el.dataset.derOrig
      }
    }

    /** 按 atSeq 找到被编辑的用户行（刷新后 DOM 是新的，要重新定位）。 */
    function hostBySeq(atSeq) {
      for (const host of userMessageHosts()) {
        let seq = null
        try {
          seq = messageSeqOf(inspectConversation(host).node)
        } catch (error) {
          seq = null
        }
        if (seq === atSeq) return host
      }
      return null
    }

    function retextHost(host, text) {
      const bubble = host.querySelector('[class*="userBubble"],[class*="_bubble"]')
      if (bubble === null) return
      if (host.dataset.derOrig === undefined) host.dataset.derOrig = bubble.textContent
      bubble.textContent = text
      host.setAttribute('data-der-retext', '1')
    }

    function flowTurn(flow) {
      const raw = flow.getAttribute && flow.getAttribute('data-chat-turn')
     ...
     * 顶层 flow（turn 级容器）。
     *
     * production 的 turn 号打在 `[data-chat-flow-kind]` 祖先上（flowInfoOf 读的
     * 就是它），而不是每行自己的 `[data-chat-flow]` 上——之前按行读 turn
     * 全是 null，这是“气泡换了但旧行藏不掉”的直接原因。
     */
    function topKinds() {
      ...
      return all.filter((el) => {
        ...
        return parent === null || parent === undefined
          ...
          || parent.closest('[data-chat-flow-kind]') === null
      })
    }

    function kindOf(host) {
      if (host === null || host === undefined || typeof host.closest !== 'function') return null
      return host.closest('[data-chat-flow-kind]') || host.closest('[data-chat-flow]')
    }

    /** 取一个 turn 容器的轮次号：先读 DOM 标记，缺失时用里面第一条用户行的 fiber。 */
    function kindTurn(kind) {
      const domTurn = flowTurn(kind)
      if (domTurn !== null) return domTurn
      try {
        const stack = kind.querySelector && kind.querySelector('[class*="userStack"]')
        const host = stack !== null && stack !== undefined
          ? (stack.closest('[class*="userRow"]') || stack)
          : null
        if (host === null) return null
        const info = inspectConversation(host)
        const turn = turnInfoOf(info.node)
        return turn && Number.isFinite(turn.turn) ? turn.turn : null
      } catch (error) {
        return null
      }
    }
    ...
    function applyMirror() {
      ...
      const records = sessionMirror(loadMirror(), sessionId).edits
      if (records.length === 0) return
      const flows = topKinds()
      clearMirrorMarks()
      for (const record of records) {
        const locateSeq = Number.isFinite(record.domSeq) ? record.domSeq : record.atSeq
        const edited = Number.isFinite(locateSeq) ? hostBySeq(locateSeq) : null
        const editedKind = kindOf(edited)
        if (edited !== null) retextHost(edited, record.text)
        for (let kindIdx = 0; kindIdx < flows.length; kindIdx++) {
          const kind = flows[kindIdx]
          if (kind === editedKind || kind.getAttribute('data-der-hidden') !== null) continue
          const turn = kindTurn(kind)
          let hide = false
          if (Number.isFinite(record.turn) && turn !== null) {
            // 只藏 [被编辑轮, hideUntil] 闭区间，且绝不碰提交时刻之后新长的 kinds。
            const inRange = record.hideUntil === null || record.hideUntil === undefined
              ? turn === record.turn
              : turn >= record.turn && turn <= record.hideUntil
            const isNew = Number.isFinite(record.kindCount) && kindIdx >= record.kindCount
            hide = inRange && !isNew
          } else if (Number.isFinite(record.editIdx) && Number.isFinite(record.kindCount)) {
            // turn 读不到时的兜底：按下标准确藏，之后新长的豁免。
            hide = kindIdx >= record.editIdx && kindIdx < record.kindCount
          } else if (editedKind !== null && typeof kind.compareDocumentPosition === 'function'
            && typeof Node !== 'undefined') {
            // 最后手段：文档顺序。只在完全没有快照时用。
            hide = (kind.compareDocumentPosition(editedKind)
              & Node.DOCUMENT_POSITION_PRECEDING) !== 0
          }
          if (hide) {
            kind.style.display = 'none'
            kind.setAttribute('data-der-hidden', '1')
          }
        }
        // 同 turn 的旧助手回复和被编辑行在同一个 turn 容器里：
        // 按文档顺序藏掉被编辑行之后的行内 flow（不需要任何 turn 标记）。
        if (editedKind !== null) {
          const editedFlow = edited !== null && typeof edited.closest === 'function'
            ? (edited.closest('[data-chat-flow]') || editedKind)
            : editedKind
          const inner = [...editedKind.querySelectorAll('[data-chat-flow]')]
          for (const flow of inner) {
            if (flow === editedFlow || flow.contains(editedFlow) || editedFlow.contains(flow)) continue
            if (flow.getAttribute('data-der-hidden') !== null) continue
            let after = false
            try {
              after = typeof flow.compareDocumentPosition === 'function' && typeof Node !== 'undefined'
                ? (flow.compareDocumentPosition(editedFlow) & Node.DOCUMENT_POSITION_PRECEDING) !== 0
                : false
            } catch (error) {
              after = false
            }
            if (after) {
              ...
              flow.setAttribute('data-der-hidden', '1')
            }
          }
        }
      }
    ...
    function scheduleMirror() {
      ...
      const run = () => {
        ...
        try {
          ...
          diag('mirror failed', { message: error && error.message })
        }
      }
    ...
    function ensureEditor() {
      ...
      editor.addEventListener('keydown', (event) => {
        ...
        // Option+Enter：啰嗦模式，成功 toast 里带出定位数据（atSeq/范围/新seq），
        // 截屏就能排查，不用开控制台。
        void commit(editor.value, { verbose: event.altKey === true })
      })
    ...
    function startEdit(host) {
      ...
      // 快照来源按可靠性排序：fiber 上拿到的 > 会话句柄 > 注入服务现取的。
      // 真机上 handles.session 常为 null，所以注入服务这条兜底不能少。
      const pending = pendingItemFor(
        handles.snapshot || session || (queueBridge && queueBridge.snapshot) || snapshotFor(handles.sessionId),
        handles.node,
        text,
      )
     ...
     * 发起「原地改写」。
     *
     * 走插件自己注册的 HTTP 端点（`POST /edit-resend/rewrite`）——**不是命令**，
     * 界面上不会出现任何中间物：改成什么，那条消息就变成什么。
     *
     * 请求里显式带上 sessionId，所以由服务端按会话定位，
     * 不依赖"当前屏幕上的输入框"，也就不可能发到别的会话去。
     *
     * @param current - 编辑目标。
     * @param text - 改写后的文本。
     * @returns 结果描述 `{ ok, reason?, detail? }`。
     */
    async function rewriteInPlace(current, text) {
      // 只在会话结束后改写：正在回答时改历史会打乱这一轮。
      if (current.inFlight) return { ok: false, reason: 'BUSY' }

      const sessionId = current.handles && current.handles.sessionId
      const atSeq = Number.isFinite(current.messageSeq) ? current.messageSeq : null
      if (!sessionId) return { ok: false, reason: 'NO_SESSION' }
      if (atSeq === null) return { ok: false, reason: 'NO_SEQ' }
      // 同一行反复改写时 fiber 上的 seq 还是最初那条（早已被盖掉），
      // 用镜像的 remap 指到最新的替换事件 seq。
      const effSeq = mirrorEffSeq(sessionId, atSeq)

      diag('rewriteInPlace', { sessionId, atSeq, effSeq, textLength: text.length })
      let response
      try {
        response = await fetch('/edit-resend/rewrite', {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ sessionId, atSeq: effSeq, text }),
        })
      } catch (error) {
        diag('rewriteInPlace fetch failed', { message: error && error.message })
        return { ok: false, reason: 'UNREACHABLE', detail: error && error.message }
      }
      ...
      try {
        ...
        return { ok: false, reason: 'BAD_RESPONSE', detail: `HTTP ${response.status}` }
      }
      diag('rewriteInPlace result', { status: response.status, payload })
      if (payload && payload.ok === true) {
        return {
          ok: true,
          backup: payload.backup,
          shadowedRange: payload.shadowedRange,
          replacementSeq: payload.replacementSeq,
          effSeq,
        }
      }
      return {
    ...
    /**
     * 取一条会话的工作区信息，用于让新建会话继承同一工作区。
     * 不同层能拿到的字段不同：可能是 cwd，也可能是 workspaceId。
     * @param current - 编辑目标。
     * @returns 传给 sessions.create 的参数，或 null。
     */
    function workspaceOf(current) {
      const candidates = [
        current && current.workspaceId,
        current && current.cwd,
        current && current.handles && current.handles.workspaceId,
        current && current.handles && current.handles.cwd,
      ]
      for (const value of candidates) {
        if (typeof value === 'string' && value !== '') {
          return value.startsWith('/') || value.includes('/') ? { cwd: value } : { workspaceId: value }
        }
      }
      // 兜底：从当前会话摘要里找 cwd。
      const sessionId = current && current.handles && current.handles.sessionId
      if (sessionsService !== null && typeof sessionsService.list === 'function' && sessionId) {
        try {
          const summaries = sessionsService.list()
          const rows = Array.isArray(summaries) ? summaries : (summaries && summaries.items) || []
          const hit = rows.find((row) => row && row.sessionId === sessionId)
          if (hit && typeof hit.cwd === 'string' && hit.cwd !== '') return { cwd: hit.cwd }
        } catch (error) {
          /* 拿不到就按空会话建，仍可用，只是要用户自己选工作区 */
        }
      }
      return null
    }
     ...
     * 取「原地改写队列消息」的调用器。
     *
     * 真机实测：UI 并不把会话/会话管理器暴露到 React props 上，所以
     * `handles.sessions` / `handles.session` 都是 undefined，只有 apply 时
     * 注入的 cordis 服务可用。conversation 是按会话作用域解析的服务，所以
     * 需要先 sessions.scope(sessionId) 再取 conversation。
     *
     * @param sessionId - 目标会话。
     * @returns (itemId, action) => Promise 的调用器，或 null。
     */
    function queueUpdateFor(sessionId) {
      // 直接注入的 conversation 服务（未作用域化）可用时优先。
      if (conversationService !== null && typeof conversationService.updateQueue === 'function') {
        return (itemId, action) => conversationService.updateQueue(itemId, action)
      }
      if (sessionsService !== null && typeof sessionsService.scope === 'function' && sessionId) {
        try {
          const actx = sessionsService.scope(sessionId)
          const scoped = actx && typeof actx.get === 'function' ? actx.get('conversation') : null
          if (scoped && typeof scoped.updateQueue === 'function') {
            return (itemId, action) => scoped.updateQueue(itemId, action)
          }
        } catch (error) {
          diag('queueUpdateFor scope failed', { message: error && error.message })
        }
      }
      return null
    }
    ...
    /**
     * 取一条会话的快照（含 queue），用于判断某条消息是否还在排队。
     * @param sessionId - 目标会话。
     * @returns 快照对象或 null。
     */
    function snapshotFor(sessionId) {
      if (sessionsService !== null && typeof sessionsService.get === 'function' && sessionId) {
        try {
          const session = sessionsService.get(sessionId)
          if (session && typeof session.getSnapshot === 'function') return session.getSnapshot()
        } catch (error) {
          diag('snapshotFor get failed', { message: error && error.message })
        }
      }
      if (sessionsService !== null && typeof sessionsService.scope === 'function' && sessionId) {
        try {
          const actx = sessionsService.scope(sessionId)
          const scoped = actx && typeof actx.get === 'function' ? actx.get('conversation') : null
          if (scoped && typeof scoped.snapshot === 'function') return scoped.snapshot()
        } catch (error) {
          /* 拿不到就当没有队列 */
        }
      }
     ...
     * @returns 是否成功新建并打开。
     */
    async function createFreshSession(current) {
      if (sessionsService === null || typeof sessionsService.create !== 'function') return false
      // 继承来源会话的工作区：空构造出来的会话会停在「选择一个工作区开始」的
      // 欢迎页，那里输入框不可用，后续就填不进去、也发不出去。
      const target = workspaceOf(current)
      let result
      try {
        result = await sessionsService.create(target === null ? {} : target)
      } catch (error) {
    ...
    async function commit(newText, options) {
      ...
      if (current.pendingId) {
        // 真机上 handles.sessions 是 undefined，队列改写必须走注入的服务。
        const update = (typeof current.queueUpdate === 'function' ? current.queueUpdate : null)
          || queueUpdateFor(current.handles && current.handles.sessionId)
        diag('commit', { branch: 'queue-edit', pendingId: current.pendingId, hasUpdater: update !== null })
        if (update === null) {
          await setComposerText(newText)
          showToast('拿不到队列改写接口，文本已放回输入框，请手动发送')
          return
        }
        try {
          await update(current.pendingId, {
            kind: 'edit',
            content: [{ type: 'text', text: newText }],
          })
        } catch (error) {
      ...
      // 路径 2：已落地的历史消息 → **原地改写**（不新建会话）。
      const atSeq = Number.isFinite(current.messageSeq) ? current.messageSeq : null
      diag('commit', { branch: 'rewrite-in-place', atSeq, inFlight: current.inFlight })

      if (atSeq === null) {
        await setComposerText(newText)
        showToast('拿不到这条消息的事件序号，无法定位改写点；文本已放回输入框')
        return
      }

      const outcome = await rewriteInPlace(current, newText)
      if (outcome.ok) {
        diag('rewrite ok', { backup: outcome.backup })
        // 模型侧已盖掉；DOM 侧由镜像接管，保证所见即所得。
        // domSeq 留的是 fiber 那行的 seq（供再次定位），atSeq 是实际发送的。
        // hideUntil 记下编辑时刻的最大轮次，之后的重答/新消息不再被藏。
        let hideUntil = null
        try {
          for (const kind of topKinds()) {
            const turn = kindTurn(kind)
            if (turn !== null && (hideUntil === null || turn > hideUntil)) hideUntil = turn
          }
        } catch (error) {
          hideUntil = null
        }
        recordMirrorEdit(current.handles && current.handles.sessionId, {
          atSeq: outcome.effSeq ?? atSeq,
          domSeq: atSeq,
          turn: current.turn,
          hideUntil,
          text: newText,
          replacementSeq: outcome.replacementSeq,
        })
        applyMirror()
        if (verbose) {
          const range = Array.isArray(outcome.shadowedRange) ? outcome.shadowedRange.join('..') : '?'
          showToast(`改写成功：atSeq=${outcome.effSeq ?? atSeq} 范围=${range} 新seq=${outcome.replacementSeq ?? '?'} turn=${current.turn ?? '?'}`, 9000)
        } else {
          showToast('已原地改写，正在重新回答…')
        }
        return
      }
      await setComposerText(newText)
      // 失败原因按「用户能做什么」翻译，而不是抛内部码。
      const REASONS = {
        BUSY: '会话正在回答中，等这一轮结束后再改写',
        NO_SESSION: '拿不到这条消息所属的会话',
        NO_SEQ: '拿不到这条消息的事件序号',
        UNREACHABLE: '连不上改写接口（插件主机半边可能没加载）',
        NO_AGENT: '这个会话当前不在活动状态，无法改写',
        REWRITE_FAILED: `改写被拒绝：${outcome.detail ?? '未知原因'}`,
        NO_SURFACE: '这个会话没有可见历史，无法定位改写点',
        NO_KICK: `历史已改写，但当前版本无法唤醒重答：${outcome.detail ?? ''}`,
        KICK_FAILED: `历史已改写，但唤醒重新回答失败：${outcome.detail ?? '未知原因'}`,
        FOLLOWUP_FAILED: `历史已改写，但唤醒重新回答失败：${outcome.detail ?? '未知原因'}`,
        BAD_REQUEST: '改写请求参数不合法',
        BAD_RESPONSE: `改写接口返回异常：${outcome.detail ?? ''}`,
      }
      const reason = REASONS[outcome.reason] ?? `改写没能发起（${outcome.reason ?? '未知'}）`
      const detail = outcome.detail ? `：${outcome.detail}` : ''
      showToast(`${reason}${detail}；文本已放回输入框`)
    }
    ...
    function apply(ctx) {
      ...
      // React 重渲染把它清掉后补回来。
      startObserving()
      // 刷新后重放视觉镜像（localStorage 里的改写记录）。
      try {
        applyMirror()
      } catch (error) {
        diag('mirror replay failed', { message: error && error.message })
      }
      const dispose = () => {
      ...
      try {
        globalThis.__dshEditResend = () => {
          const allSeqs = collectUserMessageSeqs()
          const rows = userMessageHosts().map((host, index) => {
            const info = inspectConversation(host)
            const seq = messageSeqOf(info.node)
            const turn = turnInfoOf(info.node)
            const flow = flowInfoOf(host)
            // location / turn 的键名形状：边界算不出来时一眼能看出是
            // 「没有 turn.end」还是「键名不叫这个」——这是 fork 锚点的关键依赖。
            const loc = info.node && info.node.location
            const turnObj = loc && loc.turn
            return {
              index,
              seq,
              ...
              turnEndSeq: turn ? turn.endSeq : null,
              boundarySeq: forkBoundaryBefore(info.node, index > 0 ? allSeqs[index - 1] : null),
              flowKind: flow.kind,
              还在流转: flow.kind === 'steering' || seq === null,
              hasSession: info.session !== null,
              hasSnapshot: info.snapshot !== null,
              hasInjectedServices: sessionsService !== null || conversationService !== null,
              queueFromService: snapshotFor(info.sessionId) !== null,
              hasForkAt: typeof info.forkAt === 'function',
              sessionId: info.sessionId,
              locationKeys: loc ? Object.keys(loc).join(',') : null,
              turnKeys: turnObj && typeof turnObj === 'object' ? Object.keys(turnObj).join(',') : null,
            }
          })
